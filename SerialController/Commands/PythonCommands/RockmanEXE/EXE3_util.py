...
class EXE3_util(EXE_util):

    """
    --------------------------------------------------------------------------------------
     ロックマンエグゼ３用コマンド v1.0 Release.2024/10/22
    ...
    def __init__(self, cam, gui=None):
        ...
        ###シリーズ固有値###
        self.seriesNumber = "3" #ナンバリング
        self.tmplFolder = "EXE3" #Templateフォルダ名
        self.max_row = 2    #カスタム画面の最大行数
        self.max_col = 5    #カスタム画面の最大列数
        self.normal_end = "E300"  #正常終了
        self.error_nochip_backpack = "E311" #通信対戦エラー（負けた時に相手に渡すチップがない）
    ...
    # # 引数２：認識対象チップ画像ファイル名のリスト
    # # 引数３：引数２リスト内のレギュラーチップの要素数
    # def EXE2TradeNoChip(self, winner_flag, chipArray, regular_Index):
    #     '''
    #     【前提】＠ｄｓｄｄｄ＠
    #     【戻り値】～～～～
