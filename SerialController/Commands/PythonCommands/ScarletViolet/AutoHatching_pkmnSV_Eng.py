...
class AutoHatching_pkmnSV(SV_util_box_Eng,SV_util_picnic_Eng,util_Switch_Poke):
    NAME = '@【ＳＶ】自動タマゴ孵化(SV英語版)'
    '''
    --------------------------------------------------------------------------------------
        ...
        while True :    
            ...
            while True:
                ...
                if not self.resetPositionZerogate():       
                    ...
                    continue 
                # <1>手持ちタマゴの数分、孵化を行う
                # print('◆ログ<1>hatching開始')
                self.hatching(eggs) 
                # print('◆ログ<1>hatching終了')
                # <2>孵化したポケモンをボックスに戻す
                # 色違いがいた場合数をカウントし、後続処理でボックスの逃がし処理を行う
                # print('◆ログ<2>putHatchedEggs開始　shiny_putHatchedEggs：',shiny_putHatchedEggs)
                shiny_putHatchedEggs += self.putHatchedEggs()   
                # print('◆ログ<2>putHatchedEggs終了　shiny_putHatchedEggs：',shiny_putHatchedEggs)
                # <3>タマゴを手持ちに入れ、手持ちのタマゴ数を確認する
                # print('◆ログ<3>pullEggs開始　：手持ちタマゴ数：',eggs)
                eggs = self.pullEggs()
                # print('◆ログ<3>pullEggs終了　：手持ちタマゴ数：',eggs)
                if eggs == 0:
    ...
    def countEggsInParty(self):
        ...
        """
        eggs = 0
        if self.isContainTemplate('SV_AutoHatching_Eng\menu_eggs1.png',threshold=0.9): 
            eggs=1
            if self.isContainTemplate('SV_AutoHatching_Eng\menu_eggs5.png',threshold=0.9):   
                eggs=5        
            elif self.isContainTemplate('SV_AutoHatching_Eng\menu_eggs4.png',threshold=0.9):   
                eggs=4
            elif self.isContainTemplate('SV_AutoHatching_Eng\menu_eggs3.png',threshold=0.9):   
                eggs=3
            elif self.isContainTemplate('SV_AutoHatching_Eng\menu_eggs2.png',threshold=0.9):   
                eggs=2
        print('eggs: ',eggs)
        return eggs
    ...
    def resetPositionZerogate(self):
        ...
        """
        # initialize
        map_zerogate = 'SV_AutoHatching_Eng\Ymenu_map_zerogate.png'
        # 空を飛ぶ
    ...
    def getNewEgg(self,egg=30):
        ...
        # initialize
        recipe_path='SV_AutoHatching_Eng\sandwich.png'          # 作りたいサンドイッチの画像パス
        power_path='SV_AutoHatching_Eng\sandwich_power_lv2.png' # 発動させたいパワーレベルの画像パス
    ...
    def changePokemonToParent(self):
        ...
        self.openBox()
        # move box
        self.moveBox('SV_AutoHatching_Eng\inbox_title_base.png')  
        # focus move to party          
        ...
        self.moveBox('SV_AutoHatching_Eng\inbox_title_egg.png')                 # 念のためボックス名を確認
        # check party
        if self.isContainTemplate('SV_AutoHatching_Eng\inbox_party_null_5.png',area=[0,720,0,300]):
            print('ERROR : changePokemonToParent - connot moved pkmn')
            # send LINE
            if self.debug_img_display:
                self.LINE_image('[Eng]ERROR : changePokemonToParent - connot moved pkmn')    # デバッグ用  
            return False    
        # close
        self.closeBox()     
        return True
    ...
    def changePokemonToHatching(self):
        ...
        self.moveBox('SV_AutoHatching_Eng\inbox_title_egg.png')                 # 念のためボックス名を確認
        # check party
        if not self.isContainTemplate('SV_AutoHatching_Eng\inbox_party_null_5.png',area=[0,720,0,300]):   
            print('ERROR : changePokemonToHatching - connot moved pkmn')
            # send LINE
            if self.debug_img_display:
                self.LINE_image('[Eng]ERROR :changePokemonToHatching - connot moved pkmn')    # デバッグ用
            return False
        return True
    ...
    def hatching(self,eggs=5):
        ...
        for i in range(0, eggs):
            ...
            # hatching notice
            while not self.isContainTemplate('SV_AutoHatching_Eng\message_hatching.png'):
                self.press(Button.LCLICK)  
                count += 1
                if count >= 400:
                    ...
                    return False
            # end run
            # TOM_COMMENT 遅延対策 START 3.5は安定確認
            self.wait(1.5)
    ...
    def pullEggs(self):
        ...
        while True:
            ...
            self.openBox()                     
            # move box
            if self.moveBox('SV_AutoHatching_Eng\inbox_title_egg.png')  is None: 
                print('retry')
                # retry
                self.closeBox()
                continue    
            ...
            if self.tokupokeflag:
                if self.isContainTemplate('SV_AutoHatching_Eng\inbox_egg.png',area=[0,720,0,300],threshold=0.9):  
                    print('SKIP : pullEggs - already has eggs - tokupokeflag,',self.tokupokeflag)
                    eggs = self.checkPartyEggs()  
                    return eggs 
            else:
                if self.isContainTemplate('SV_AutoHatching_Eng\inbox_egg.png',area=[0,720,0,300]):  
            ...
            if self.tokupokeflag:
                position_egg = self.isContainTemplatePosition('SV_AutoHatching_Eng\inbox_egg.png',threshold=0.8)
                # print('[debug]pullEggs - position_egg_get - tokupokeflag',self.tokupokeflag,'position_egg=',position_egg)
            else:
                position_egg = self.isContainTemplatePosition('SV_AutoHatching_Eng\inbox_egg.png')
                # print('[debug]pullEggs - position_egg_get','position_egg=',position_egg)
            # comment
            # print('◆ボックス内に卵があるか：',position_egg)
            # comment
            ...
            if not self.moveCellFocus(row=1,column=target_column) :
                ...
                # LINE送信   
                if self.debug_img_display:
                    self.LINE_image('[Eng]ERROR : pullEggs - moveCellFocus')    # デバッグ用：LINE通知不要なら削除
                # retry
                self.closeBox()
                continue
            # pull
            ...
            if self.tokupokeflag:
                if not self.isContainTemplate('SV_AutoHatching_Eng\inbox_egg.png',area=[0,720,0,300],threshold=0.9):      
                    print('ERROR : pullEggs - cannot move eggs - tokupokeflag,',self.tokupokeflag)
                    # LINE送信   
                    if self.debug_img_display:
                        self.LINE_image('[Eng]ERROR : pullEggs - cannot move eggs')    # デバッグ用：LINE通知不要なら削除
                    # retry
                    self.closeBox()
                    continue    
            else:
                if not self.isContainTemplate('SV_AutoHatching_Eng\inbox_egg.png',area=[0,720,0,300]):      
        ...
        while True:
            ...
            if self.tokupokeflag:
                ...
                if self.isContainTemplate('SV_AutoHatching_Eng\inbox_egg.png',area=[0,720,0,300]):  
                    ...
                    return shiny_count            
            # party pkmn check                           
            if self.isContainTemplate('SV_AutoHatching_Eng\inbox_party_null_5.png',area=[0,720,0,300],threshold=0.9):
            ...
            for i in range(1, 7):
                while not self.moveCellFocus(row=i,column=0):
                    ...
                    # send LINE
                    if self.debug_img_display:
                        self.LINE_image('[Eng]ERROR : putHatchedEggs - moveCellFocus')    # デバッグ用
                    # retry
                    self.closeBox()
                    self.openBox()
                # cell check
                ...
                if flag[0]: 
                    ...
                    # if self.use_LINEnotice:
                    #     self.LINE_image('[Eng]*** shiny(putHatchedEggs) ***')    # LINE通知不要なら削除 
                elif flag[1]:
                    # statusmatch
                    self.IV_flag = True    
                    print('*** status match ***')                
                    if self.use_LINEnotice:
                        self.LINE_image('[Eng]*** status match ***')    # LINE通知不要なら削除  
            # find null row
            while True:
                position_null_space = self.isContainTemplatePosition('SV_AutoHatching_Eng\inbox_row_space.png',threshold=0.8,
                                                                        mask_path='SV_AutoHatching_Eng\mask_boxarea.png')
            ...
            if not self.isContainTemplate('SV_AutoHatching_Eng\inbox_party_null_5.png',area=[0,720,0,300]):
                ...
                # send LINE
                if self.debug_img_display:
                    self.LINE_image('[Eng]ERROR : putHatchedEggs - cannot move pkmn')    # デバッグ用
                # retry
                self.closeBox() 
                continue
            # before box            
