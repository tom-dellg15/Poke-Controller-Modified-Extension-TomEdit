...
class SV_util_picnic_Eng(SV_util_Eng):
    ...
    def openPicnic(self):
        ...
        Xメニューを開いてピクニックを選択する
        """
        print('_method(SV_util_picnic_Eng)_openPicnic')
        # reset direction
        self.press(Direction.DOWN, duration=0.2, wait=0.5)  
        # x menu
        self.openXmenu()  
        # select picnic
        while not self.isContainTemplate('SV_util_Eng\_xmenu_focus_picnic.png'):
    ...
    def closePicnic(self):
        ...
        """
        print('_method(SV_util_picnic_Eng)_closePicnic')
        # close
        while self.isContainTemplate('SV_util_Eng\_picnic_endcommand.png'):
        ...
        # skip lvup message
        # x menu
        while not self.isContainTemplate('SV_util_Eng\_xmenu.png',use_binary=True, threshold_binary=190):   
    ...
    def makeSandwich(self,recipe_path,power_path,repeatcount_max=30):
        ...
        """
        print('_method(SV_util_picnic_Eng)_makeSandwich')
        repeatcount = 0
        # start cook
        self.press(Button.A, wait=1)    
        self.press(Button.A, wait=6)  
        # check started cooking
        if not self.isContainTemplate('SV_util_Eng\_sandwich_recipetitle.png'):
            print('ERROR: makeSandwich -  cannot open recipe')
            # debug
            if self.debug_img_display:
                self.LINE_image('[Eng]ERROR: makeSandwich - cannot open recipe') 
            return False
        # choice sandwich        
        ...
        for i in range(0, 3):
            ...
            while True:            
                # position adjustment
                position = self.isContainTemplatePosition('SV_util_Eng\_sandwich_cursor.png')
                if position is None:
        ...
        # wait power
        repeatcount = 0
        while not self.isContainTemplate('SV_util_Eng\_sandwich_power.png', use_binary=True, threshold_binary=190, mask_path='SV_util_Eng\_mask_picnic_sandwichpower.png'):
            ...
            if repeatcount >150:
                ...
                # debug
                if self.debug_img_display:
                    self.LINE_image('[Eng]ERROR: makeSandwich - makeSandwich time over')   
                return False
        self.wait(1)
        # check egg power
        if not self.isContainTemplate(power_path,threshold=0.8, use_binary=True, threshold_binary=190, mask_path='SV_util_Eng\_mask_picnic_sandwichpower.png'):
            print('ERROR: makeSandwich - sandwich power is not get')
            # debug
            if self.debug_img_display:
                self.LINE_image('[Eng]ERROR: makeSandwich - sandwich power is not get')   
            return False
        # A repeat
        while not self.isContainTemplate('SV_util_Eng\_picnic_endcommand.png'):
            self.press(Button.A,wait=0.5)
        self.wait(2)
        return True
    ...
    def checkBasket(self,egg=30):
        ...
        '''
        print('_method(SV_util_picnic_Eng)_checkBasket')
        egg_count=0
        wait_count = 0
        # check basket
        self.press(Button.A,wait=1) 
        if not self.isContainTemplate('SV_util_Eng\_picnic_message_basket_check.png'):
            print('ERROR: checkBasket - cannot walk to basket')
            # debug
            if self.debug_img_display:
                self.LINE_image('[Eng]ERROR: checkBasket - cannot walk to basket')  
            return False
        # repeat check basket    
        while True:
            # egg found message
            if self.isContainTemplate('SV_util_Eng\_picnic_message_egg_found.png',threshold=0.9):
                wait_count = 0
                if egg_count<egg:
                    # get egg
                    egg_count += 1
            ...
            elif self.isContainTemplate('SV_util_Eng\_picnic_message_egg_notfound.png'):
                wait_count += 1
                if wait_count >= 10:                    # 10bak
                    print('ERROR: checkBasket - checkBasket time over')
                    # debug
                    if self.debug_img_display:
                        self.LINE_image('[Eng]ERROR: checkBasket - checkBasket time over') 
                    return False
                # skip messsage
                self.press(Button.B,wait=0.5)
                if egg_count>=egg:
