from .SV_util_Eng import SV_util_Eng
from Commands.Keys import KeyPress, Button, Direction, Stick, Hat


class SV_util_box_Eng(SV_util_Eng):
    ...
    def __init__(self, cam, gui=None):
        ...
        self.debug_messege_display = True  # debug用 True=処理確認用のメッセージをログに出力
    ...
    def openBox(self):
        ...
        Xメニューを開いてボックスを選択する
        """
        print("_method(SV_util_box_Eng)_openBox")
        # check box open
        if self.isContainTemplate("SV_util_Eng\_box.png"):
            print("alrady opened Box")
            return
        # x menu
        self.openXmenu()
        # select box
        while not self.isContainTemplate("SV_util_Eng\_xmenu_focus_box.png"):
            self.press(Direction.DOWN, wait=0.5)
        # wait box
        while self.isContainTemplate("SV_util_Eng\_xmenu_focus_box.png"):
    ...
    def closeBox(self):
        ...
        ボックスを終了する
        """
        print("_method(SV_util_box_Eng)_closeBox")
        # check box open
        if self.isContainTemplate("SV_util_Eng\_box.png"):
            self.closeXmenu()
        else:
    ...
    def moveBox(self, boxtitle_path):
        ...
        '''
        """
        print("_method(SV_util_box_Eng)_moveBox")
        # box move
        for i in range(0, 30):
    ...
    def moveCellFocus(self, row, column):
        ...
        if self.debug_messege_display:
            print("_method(SV_util_box_Eng)_moveCellFocus row=", row, "column=", column)
        # focus move
        while True:
            # check background color
            cursor_position = self.isContainTemplatePositionBGR(
                "SV_util_Eng\_box_focus_cell_bg.png",
                threshold=0.8,
                lower=[0, 190, 200],
                upper=[80, 255, 255],
                mask_path="SV_util_Eng\_mask_boxarea_partyarea.png",
            )  # 環境に合わせてBGR値を設定ください
            # not found
            if cursor_position is None:
                # check background color (pull)
                cursor_position = self.isContainTemplatePositionBGR(
                    "SV_util_Eng\_box_focus_cell_bg_pull.png",
                    threshold=0.8,
                    lower=[0, 190, 200],
                    upper=[80, 255, 255],
                    mask_path="SV_util_Eng\_mask_boxarea_partyarea.png",
        ...
        for row in range(1, 6):
            ...
            for column in range(1, 7):
                ...
                if self.isContainTemplate(
                    "SV_util_Eng\_box_cell_null.png", area=target_area
                ):
                    return [row, column]
        return None
    ...
    def releasePkmn(self):
        ...
        # target is not null
        if self.isContainTemplate("SV_util_Eng\_box_status_lv.png", area=status_lv_area):
            while True:
                # TOM_COMMENT 秒数増加 遅延対策
                self.wait(1.0)
                self.press(Button.A, wait=1.0)
                if self.isContainTemplate("SV_util_Eng\_box_message_pkmnmenu.png"):
                    break
            while True:
                self.press(Direction.UP, wait=0.3)
                if self.isContainTemplate(
                    "SV_util_Eng\_box_pkmnmenu_nigasu.png", use_gray=False, threshold=0.8
                ):
                    break
            while True:
                self.press(Button.A, wait=1)
                if self.isContainTemplate("SV_util_Eng\_box_pkmnmenu_no.png"):
                    break
            while True:
                self.press(Direction.UP, wait=0.3)
                if not self.isContainTemplate("SV_util_Eng\_box_pkmnmenu_no.png"):
                    break
            while True:
                self.press(Button.A, wait=0.3)
                if not self.isContainTemplate(
                    "SV_util_Eng\_box_status_lv.png", area=status_lv_area
                ):
                    break
            self.wait(0.5)
    ...
    def putNextBox(self):
        ...
        count_move_box = 0
        # pull
        # TOM_COMMENT _box_pullPkmn.pngの画像を差し替えた
        while not self.isContainTemplate("SV_util_Eng\_box_pullPkmn.png"):
        ...
        if self.moveCellFocus(row=target_area[0], column=target_area[1]) == False:
            ...
            return False
        # TOM_COMMENT 遅延対策 24/9/8 画像差し替えで解決した？ので、コメントアウト
        #print("▼遅延対策(Aボタン押すまで3秒待つ)▼")
        #self.wait(0.5)
        self.press(Button.A, wait=1)
        # before box
    ...
    def checkCell(self, statuscheck_flag=False):
        ...
        # debug message
        if self.debug_messege_display:
            print("_method(SV_util_box_Eng)_checkCell")
        shiny_flag = False
        status_flag = False
        # cell check
        if self.isContainTemplate(
            "SV_util_Eng\_box_status_lv.png", area=[0, 100, 1100, 1280]
        ):
            # shiny check
            if self.isContainTemplate(
                "SV_util_Eng\_shiny_mark.png", area=[0, 100, 1100, 1280]
            ):
                shiny_flag = True
            else:
                if statuscheck_flag:
    ...
    def checkCellPkmnStatus(self):
        ...
        # debug message
        if self.debug_messege_display:
            print("_method(SV_util_box_Eng)_checkCellPkmnStatus")
        # mode
        while not self.isContainTemplate("SV_util_Eng\_box_mode_judge.png"):
    ...
    def checkStatus(self, status=[31, 31, 31, 31, 31, 31]):
        ...
        for i in range(0, 6):
            ...
            if status[i] == 31 and not self.isContainTemplate(
                "SV_util_Eng\_box_status_31.png", area=statusArea[i]
            ):
                return False
            # 0 check
            elif status[i] == 0 and not self.isContainTemplate(
                "SV_util_Eng\_box_status_0.png", area=statusArea[i]
            ):
                return False
        return True
    ...
    def checkBoxShinyOrStatus(
        ...
        【前提】BOXを開いた状態から
        """
        print("_method(SV_util_box_Eng)_checkBoxShinyOrStatus")
        print(
            "release_flag : ",
            release_flag,
            " / statuscheck_flag : ",
        ...
        while True:
            ...
            if not self.moveCellFocus(row=now_row, column=now_column):
                ...
                if self.debug_img_display:
                    self.LINE_image(
                        "[Eng]ERROR : checkBoxShinyOrStatus - moveCellFocus"
                    )  # デバッグ用：LINE通知不要なら削除
                # retry
                now_row = 1
                now_column = 1
            ...
            if flag[0]:
                ...
                if self.use_LINEnotice:
                    self.LINE_image("[Eng]*** shiny ***")  # LINE通知不要なら削除
                if pkmnmove_flag:
            ...
            if now_row >= 6:
                break
                #Debug用
                print("▼checkBoxShinyOrStatusループ終了▼")
        # flag check
        if shiny_count <= 0:
            print("*** not shiny ***")
            # send LINE
            #comment 不要
            #if self.debug_img_display:
                #self.LINE_image("[Eng]*** not shiny ***")  # デバッグ用：LINE通知不要なら削除
        return shiny_count
